<template>
  <div style="position: relative; margin:20px 0 0 5% ">
    ...
    <div>
      ...
      <el-button @click="clearTest" type="text" title="清除提交记录，重新获取题库所有题目">清除记录</el-button>
    </div>

    <div style="display: inline-block; width: 30%; vertical-align: top">
      <el-button @click="submitTestCS" title="提交做题记录，下次进入将不再获取已做过的题目">提交</el-button>
      <el-pagination
          small
    ...
    <div style="display: inline-block; width: 30%; margin-left: 20px; vertical-align: top">
      <el-button @click="submitTestFS" title="提交做题记录，下次进入将不再获取已做过的题目">提交</el-button>
      <el-pagination
          small
    ...
    <div style="display: inline-block; width: 30%; margin-left: 20px; vertical-align: top">
      <el-button @click="submitTestSAS" title="提交做题记录，下次进入将不再获取已做过的题目">提交</el-button>
      <el-pagination
          small
...
export default {
  ...
  methods:{
    ...
    submitTestFS(){
      this.isSubmitFS=true
      this.uploadForm.progressC=(this.paginationCS.currentPage-1)*this.paginationCS.pageSize
      this.uploadForm.progressF=this.paginationFS.currentPage*this.paginationFS.pageSize
      this.uploadForm.progressSA=(this.paginationSAS.currentPage-1)*this.paginationSAS.pageSize
      request.post('uploadProgress',this.uploadForm).then(res=>{
    ...
    submitTestSAS(){
      this.isSubmitSAS=true
      this.uploadForm.progressC=(this.paginationCS.currentPage-1)*this.paginationCS.pageSize
      this.uploadForm.progressF=(this.paginationFS.currentPage-1)*this.paginationFS.pageSize
      this.uploadForm.progressSA=this.paginationSAS.currentPage*this.paginationSAS.pageSize
      request.post('uploadProgress',this.uploadForm).then(res=>{
    ...
    initialPage(){
      console.log(this.questionBank.question_bank_no)
      request.post('/findProgressQuestionsByQuestionBankNo',{
        stu_no:this.user.stu_no,
        question_bank_no:this.questionBank.question_bank_no,
        progress:0
      }).then(res=>{
