...
export default {
  ...
  methods: {
    ...
    send() {
      ...
      if (!this.text) {
        ...
        if (typeof (WebSocket) == "undefined") {
          ...
          let message = {from: this.user.stu_name+this.user.stu_no, to: this.chatUser, text: this.text}
          socket.send(JSON.stringify(message));  // 将组装好的json发送给服务端，由服务端进行转发
          this.messages.push({user: this.user.stu_name+this.user.stu_no, text: this.text})
          // 构建消息内容，本人消息
          this.createContent(null, this.user.stu_name+this.user.stu_no, this.text)
          this.text = '';
        }
      }
    },
    ...
    init() {
      ...
      let username = this.user.stu_name+this.user.stu_no;
      this.headUrl1=this.FileApi+'/Pictures/'+this.user.stu_photourl;
      let _this = this;
      if (typeof (WebSocket) == "undefined") {
        console.log("您的浏览器不支持WebSocket");
      } else {
        console.log("您的浏览器支持WebSocket");
        let socketUrl = "ws://10.236.11.68:8080/imserver/" + username;
        if (socket != null) {
          socket.close();
          socket = null;
        }
