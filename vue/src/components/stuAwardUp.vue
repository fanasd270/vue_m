...
export default {
  ...
  methods:{
    ...
    onSubmit(){
      if(this.awardForm.award_info_year===''){
        this.$message.warning('认定时间不能为空')
        return
      }
      if(this.awardForm.award_info_name===''){
        this.$message.warning('获奖名称不能为空')
        return
      }
    ...
    //表单关闭
    paperHandleClose(){
      this.awardForm.award_info_name= ""
      this.awardForm.award_info_unit= ""
      this.awardForm.award_info_rank= ""
      this.awardForm.award_info_time= ""
      this.awardForm.award_info_year= ""
