      ...
      <div v-for="(m,index) in projectToDo">
        <transition name="el-fade-in-linear">
          <el-card class="box-card" style="margin: 10px 5px 0 5px" v-if="projectToDoShow[index]">
            <span style="font-weight: bold; margin-right: 2%">姓名：{{m.scholarship_info_stu_name}}</span>
            <span style="font-weight: bold;">学号：{{m.scholarship_info_stu_no}}</span>
            <el-descriptions style="padding: 10px 5px 0 5px" :column=4>
              ...
              <el-descriptions-item label="证明材料:"><span style="color:cornflowerblue;" @click="downloadProject(m.scholarship_info_supporting_materials)">点击下载</span></el-descriptions-item>
            </el-descriptions>
      ...
      <div v-for="(m,index) in projectDid">
        <transition name="el-fade-in-linear">
          <el-card class="box-card" style="margin: 10px 5px 0 5px" v-if="projectDidShow[index]">
            ...
            <span style="margin-left: 5px">认定时间:</span>
            <span style="color:cornflowerblue;">{{m.scholarship_info_year}}</span>
            <el-button style="margin-left: 5px" v-if="m.scholarship_info_status==='1'" @click="rejectProjectDid(index)">驳回</el-button>
          </el-card>
        </transition>
      </div>
    </el-scrollbar>
...
export default {
  ...
  methods:{
    ...
    rejectProjectDid(index){
      request.post('/refuse_scholarship',this.projectDid[index]).then(res=>{
